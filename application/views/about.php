<?php defined('BASEPATH') OR exit('No direct script access allowed'); ?>

   
<div id="header-holder" class="innerpage about lighter">

    <div id="top-content" class="container-fluid">
        <div class="container">
            <div class="row">
                <div class="col-sm-12">
                    <div class="page-title">
                    <?php if ($page->title_active == 1): ?>
                        <h1 class="pageTitle"><?php echo html_escape($page->title); ?></h1>
                    <?php endif; ?>
                    </div>
                </div>
            </div>
        </div>
    </div>

</div>




<div id="sub-menu-about">
<div class="container">
<div class="support-links container">
    <div class="row">
        <div class="col-sm-6 hylink-holder">
            <div class="hylink-box">
                <div class="icon"><img src="<?php echo base_url();?>assets/images/info.svg" alt=""></div>
                <a href="#sambutan-direktur" class="link">Sambutan Direktur</a>
            </div>
        </div>
        <div class="col-sm-6 hylink-holder">
            <div class="hylink-box">
                <div class="icon"><img src="<?php echo base_url();?>assets/images/chat.svg" alt=""></div>
                <a href="#visi-misi" class="link">Visi &amp; Misi</a>
            </div>
        </div>
        <div class="col-sm-4 hylink-holder">
            <div class="hylink-box">
                <div class="icon"><img src="<?php echo base_url();?>assets/images/chat.svg" alt=""></div>
                <a href="#jajaran-komisaris" class="link">Jajaran Komisarins</a>
            </div>
        </div>
        <div class="col-sm-4 hylink-holder">
            <div class="hylink-box">
                <div class="icon"><img src="<?php echo base_url();?>assets/images/chat.svg" alt=""></div>
                <a href="#jajaran-direksi" class="link">Jajaran Direksi</a>
            </div>
        </div>
        <div class="col-sm-4 hylink-holder">
            <div class="hylink-box">
                <div class="icon"><img src="<?php echo base_url();?>assets/images/chat.svg" alt=""></div>
                <a href="#legalitas" class="link">Legalitas</a>
            </div>
        </div>
    </div>
</div>
</div>
</div>


<div id="sambutan-direktur" class="features container-fluid">
    <div class="container">
        <div class="row">
            <div class="col-sm-5 sambutan">
                <div class="img-holder">
                    <img src="<?php echo base_url();?>assets/images/logo.png" alt="">
                </div>
            </div>
            <div class="col-sm-7 def-aligned">
                <div class="feature-info">
                <div class="main-heading"><h2><img src="<?php echo base_url();?>assets/images/logo-small.png">Kata Sambutan</h2></div>
                    <div class="feature-text greeting-about-page">
                        <p>Assalamualaikum warahmatullahi wabarakatuh</p>
                        <p>Dengan Iman,</p>
                        <p>Kami adalah PT. Caplang Energi Sejahtera, berbasis di Banjarmasin, indonesia. sangat optimis akan berhasil di depan, dengan menetapkan target yang tinggi dan sesuai dengan perkembangan ekonomi, kami pmasaercaya bahwa itu akan meningkatkan kinerja kami untuk lebih memuaskan. Dalam upaya ini, Perusahaan telah membentuk pondasi yang kuat dan siap menjadi pemenang di berbagai kompetisi bisnis.</p>
                        <p>Dengan memprioritaskan kepuasan pelanggan dan berdasarkan pada hukum agama, kami selalu percaya bahwa semua perusahaan memiliki hak untuk menjadi besar.</p><p></p>
                        <div class="salam">
                        <p>Salam Hormat</p>
                        <p>M. Ridho Zulfadlin</p>
                        </div>
                        
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>






<div id="visi-misi" class="sub-pricing container-fluid">
    <div class="container pull-top">
        <div class="pricing-holder">
            <div class="row">

            <div class="main-heading"><h2><img src="<?php echo base_url();?>assets/images/logo-small.png">Visi &amp; Misi</h2></div>

                <div class="col-sm-12 col-md-6">
                    <div class="pricing-box inner visi">
                        <div class="pricing-title">VISI</div>
                        <div class="pricing-price">
                        </div>
                        <div class="c">
                            <p>Untuk menjadi perusahaan perdagangan kelas dunia</p>
                        </div>
                    </div>
                </div>

                <div class="col-sm-12 col-md-6">
                    <div class="pricing-box inner misi">
                        <div class="pricing-title">MISI</div>
                        <div class="pricing-price">
                        </div>
                        <div class="c">
                        <ol>
                            <li>Memperkuat kenaikan pangsa pasar dengan layanan solusi total.</li>
                            <li>Meningkatkan efisiensi perusahaan di semua lini.</li>
                            <li>Untuk meningkatkan kesejahteraan karyawan.</li>
                            <li>Memperoleh kebebasan finansial dalam mendukung semua kegiatan perusahaan dan investasi</li>
                            <li>Meningkatkan pencapaian laba perusahaan</li>
                        </ol>
                        </div>
                    </div>
                </div>

             
            </div>
        </div>
    </div>
</div>



<div id="jajaran-komisaris" class="features container-fluid">
    <div class="container">
        <div class="row">
            <div class="main-heading"><h2><img src="<?php echo base_url();?>assets/images/logo-small.png">Jajaran Komisaris</h2></div>
            <div class="col-sm-5 section-jajaran">
                <div class="img-holder">
                    <img src="<?php echo base_url();?>assets/images/management/cropt/komisaris-utama.jpg" alt="">
                </div>
            </div>
            <div class="col-sm-7 def-aligned">
                <div class="feature-info">
                <div class="feature-title">Adrin Saputra Alamsjah</div>
                    <div class="name">Komisaris Utama</div>
                    <div class="feature-text desc">
                        Kelahiran pada 6 Juni 1989. Berdomisili di Jakarta, Indonesia. Berpengalaman dalam bidang perdagangan bahan bakar dan telah banyak menduduki posisi strategis di berbagai perusahaan perdagangan BBM serta berhasil dalam mengembangkan berbagai bisnis di Indonesia. Menjabat sebagai Komisaris Utama PT. Caplang Energi Sejahtera sejak 2019.
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>


<div id="jajaran-direksi" class="features container-fluid">
    <div class="container">
        <div class="row">
            <div class="main-heading"><h2><img src="<?php echo base_url();?>assets/images/logo-small.png">Jajaran Direksi</h2></div>
            <div class="jajaran">
            <div class="col-sm-5 section-jajaran">
                <div class="img-holder">
                    <img src="<?php echo base_url();?>assets/images/management/cropt/president-director.jpg" alt="">
                </div>
            </div>
            <div class="col-sm-7 def-aligned">
                <div class="feature-info">
                <div class="feature-title">Muhammad Ridho Zulfadlin</div>
                    <div class="name">Direktur Utama</div>
                    <div class="feature-text desc">
                    Lahir pada 15 Juli 1989. Berdomisili di Banjarmasin, Indonesia. Lulus dari Al-Zaytun International School & University dan berhasil mengembangkan berbagai bisnis di Indonesia. Menjabat sebagai Direktur Utama PT. Caplang Energi Sejahtera sejak 2019 dan saat ini masih menjabat sebagai Penasehat Direktur PT. Giri Jaladhi Wana.
                    </div>
                </div>
            </div>
            </div>

            <div class="jajaran">
            <div class="col-sm-5 section-jajaran">
                <div class="img-holder">
                    <img src="<?php echo base_url();?>assets/images/management/cropt/director.jpg" alt="">
                </div>
            </div>
            <div class="col-sm-7 def-aligned">
                <div class="feature-info">
                <div class="feature-title">Khairul Anshari</div>
                    <div class="name">Direktur</div>
                    <div class="feature-text desc">
                    Lahir pada 13 November 1988. Berdomisili di Banjarmasin, Indonesia. Lulus dari Universitas Lambung Mangkurat, pakar sistem transportasi moda darat dan laut serta berhasil mengembangkan berbagai bisnis di Indonesia. Menjabat sebagai Direktur PT. Caplang Energi Sejahtera sejak 2019 dan saat ini masih menjabat sebagai Direktur PT. Baramega Indonesia Suryaalam.
                    </div>
                </div>
            </div>
            </div>

        </div>
    </div>
</div>



<div id="legalitas" class="features container-fluid">
    <div class="container">
        <div class="row">
            <div class="main-heading">
                <h2><img src="<?php echo base_url();?>assets/images/logo-small.png">Legalitas</h2>
                <p>PT. Caplang Energi Sejahtera</p>
            </div>


            <div class="col-lg-12 col-sm-12 legalitywrap">
                <div class="legality">
                    <div class="c">
                        <h5>AKTA PENDIRIAN</h5>
                        <p>Nomor : 01 Tanggal 01 Februari 2019</p>
                        <p>Notaris : Said Ahmad, SH</p>
                    </div>
                </div>
                <div class="legality">
                <div class="c"><h5>SKMENKUMHAM</h5><p>AHU-0006986.AH.01.01.TAHUN 2019</p></div>                   
                </div>
                <div class="legality">
                    <div class="c"><h5>SIUP : 503.33-006/SIUP.ME-11/DPMPTSP/2019</h5></div>
                </div>
                <div class="legality">
                    <div class="c"><h5>SKTU : 503-0890_II.SK/SKTU-B.Sejati__II./DPMPTSP/2019</h5></div>
                </div>
                <div class="legality">
                    <div class="c"><h5>NIB : 9120100211146</h5></div>
                </div>
                <div class="legality">
                    <div class="c"><h5>NPWP : 90.413.429.3-736.000</h5></div>
                </div>
            </div>

        </div>
    </div>
</div>